...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // TRATA O PREÇO: remove símbolos e converte para float
    $preco = str_replace(['R$', ' '], '', $_POST['preco']); // remove "R$" e espaços
    $preco = str_replace(',', '.', $preco); // troca vírgula por ponto
    $preco = floatval($preco); // converte para número decimal

    // VALIDA CAMPOS OBRIGATÓRIOS
    if (empty($nome) || empty($fornecedor) || empty($descricao) || empty($subcategoria) || empty($estoque) || $_POST['preco'] === '') {
    ...
    try {
        ...
        $stmt->bindParam(':preco', $preco); // como já é float, não precisa PDO::PARAM_INT
...
<form action="" method="post" enctype="multipart/form-data">
    ...
    <input type="number" name="estoque" id="estoque" placeholder="50 unidades" required>  unidades<p>
